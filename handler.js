...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (opts['gconly'] && !m.chat.endsWith('g.us'))
            return setTimeout(()=>{teslagod.updateBlockStatus(m.sender,'block')},1000)
        ...
        for (let name in plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (m.chat in db.data.chats || m.sender in db.data.users) {
                    let chat = db.data.chats[m.chat]
                    let user = db.data.users[m.sender]
                    if (name != 'desbanear_grupo.js' && chat?.isBanned)
                        return // Except this
                    if (name != 'desbanear_usuario.js' && user?.banned)
                        return
                }
                ...
                if (!isPrems && plugin.limit && db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `[ ! ] Te quedaste sin límites para usar algunas funciones T_T, puede comprar mas límites usando este comando:\n\n${usedPrefix}comprar`, m)
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || Connection.conn.welcome || '⚡ *Bienvenid@ @user a este grandioso grupo!*\n🍷 _*Espero y te agrade tu estancia aqui, no olvides respetar a los participantes y las reglas*_ ;)\n\n📜 *Normas del grupo actualmente :* \n'+mas+'\n@desc').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '-') :
                            (chat.sBye || this.bye || Connection.conn.bye || '[ ! ] C fue alv : @user')).replace('@user', '@' + user.split('@')[0])
                        this.sendFile(id, pp, 'bnvnd.jpeg', text, {key: {participant: "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "51995386439-1616169743@g.us","inviteCode": "m","groupName": "P", "caption": action === 'add' ? 'Nuevo participante! 🥳🎉' : 'Menos un participante 😪', 'jpegThumbnail': fs.readFileSync('./multimedia/imagenes/mylogo.jpg') }}}, false, { contextInfo: { mentionedJid: [user],externalAdReply:{title: action === 'add' ? 'Fecha de ingreso | '+fesha : 'Fecha de salida | '+fesha,"body": '[ ZonaBot/'+Intl.DateTimeFormat().resolvedOptions().timeZone+' ]',"previewType":"PHOTO","thumbnailUrl": ppg,"sourceUrl":`https://youtube.com/channel/UC_Pp8pT9vbT15k5_-i6oseA?sub_confirmation=1`}} })
                    }
                }
            }
            break
...
export async function deleteUpdate(message) {
    ...
    if (Array.isArray(message.keys) && message.keys.length > 0) {
        const tasks = await Promise.allSettled(message.keys.map(async (key) => {
            ...
            await this.reply(key.remoteJid, `*[ MENSAJE ELIMINADO ]*
            
🧬 Tipo de mensaje: _${mtype}_
🚮 Usuario: _@${participant.split`@`[0]}_
📆 Fecha: _${dfecha}_

Para desactivar está funcion, use el comando
*.apagar delete*
...
global.mcarga = (type, m, conn) => {
    let msg = {
        proceso: '_Procesando, por favor espere..._'
    }[type]
    if (msg) return m.reply(msg)
}
