...
let handler = async (m, { conn, usedPrefix, command, text }) => {
    ...
    conn.sendMessage(m.chat, { audio: audio.data, contextInfo: {externalAdReply:{title: `🎞️ ${NombreBot} 🎶`,"body": `Convertido a audio ✓`,"previewType":"PHOTO","thumbnail": fs.readFileSync('./multimedia/imagenes/mylogo.jpg'),"sourceUrl": redes[Math.floor(Math.random() * redes.length)]}}, mimetype: 'audio/mp4', fileName: `${text ? text : 'audio'}.mp3` }, { quoted: m })
}

handler.help = ['amp3']
handler.tags = ['conversor']
handler.command = /^a(mp3|a(udio)?)$/i
